...
- Store catalog and inventory observations emit SwiftData-backed values first, refresh their authoritative remote lists, replace the caches, and publish those writes through the same retained Combine streams. Successful purchases also update the inventory cache immediately.
